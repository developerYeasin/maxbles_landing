...
const PortfolioItemPage = () => {
  ...
  useEffect(() => {
    const getPortfolioItem = async () => {
      try {
        ...
        setError(
          "Failed to load portfolio item. It might not exist or there was a server error."
        );
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const metaDescription = item
    ? truncateText(stripHtml(item.description), 160)
    : "";
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-3">
                {item.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="bg-primary/10 text-primary dark:bg-primary-foreground/10 dark:text-primary-foreground px-4 py-2 rounded-full text-sm font-medium"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="flex flex-wrap gap-4">
                ...
                {item.github_url && (
                  ...
                  >
                    <Button
                      variant="outline"
                      className="group relative border-gray-300 text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800 shadow-md hover:scale-105 transition-transform overflow-hidden"
                    >
          ...
          {item.live_url && isEmbeddable(item.live_url) && (
            <div className="mt-10 mb-8">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-4 text-center">
                Note: Some websites may prevent embedding in an iframe due to
                security policies (X-Frame-Options).
              </p>
            </div>
          )}
